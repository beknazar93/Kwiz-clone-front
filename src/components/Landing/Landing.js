...
const Landing = () => {
  ...
  const handlePinSubmit = () => {
    ...
    axios
      ...
      .then((res) => {
        const students =
          res.data && res.data.length > 0
            ? res.data[0].levels.flatMap((level) => level.participants || [])
            : [];

        // Всегда переходим к шагу ввода ника, без проверки пустого списка
        setAllowedStudentsLocal(students);
        dispatch(setAllowedStudents(students));
        dispatch(selectedPin(Number(trimmedPin)));
        setStep(2);
      ...
      .catch(() => {
        // При ошибке загрузки просто переходим дальше
        setAllowedStudentsLocal([]);
        dispatch(setAllowedStudents([]));
        dispatch(selectedPin(Number(trimmedPin)));
        setStep(2);
      })
      .finally(() => {
  ...
  const handleNicknameSubmit = () => {
    ...
    // Убираем проверку по списку allowedStudents — любой ник можно ввести
    dispatch(handleNickname(trimmedNickname));
    navigate("/player");
  };
      ...
      <div className="quiz__container">
        ...
        {step === 1 ? (
          ...
          <>
            ...
            />
            <datalist id="nickname-suggestions">
              {allowedStudents.map((student, idx) => (
